...
def get_damage(
    ...
    distance = get_distance(hazard_position, ship_position)

    # Check if ship is safe from hazard
    s = hazard.safe_dist  # aplicar o fator 1/2 em "s" faz o barco sobreviver!
    safe = distance >= s  # retorna bool

    # Compute the actual damage
    damage = 0 if (safe) else (s - distance) / s
